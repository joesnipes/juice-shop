...
exports.isAdmin = () => (req, res, next) => {
  const user = exports.authenticatedUsers.from(req)
  if (user && user.data && user.data.isAdmin === true) {
    return next()
  }
...
exports.isRedirectAllowed = url => {
  try {
    ...
    for (let allowedUrl of redirectWhitelist) {
      ...
      if (target.protocol === allowedTarget.protocol && target.hostname === allowedTarget.hostname && target.pathname === allowedTarget.pathname) {
        return true
      }
    }
  } catch (err) {
