...
Generate and store a comprehensive security threat model for a repository using the STRIDE methodology. This skill analyzes the codebase architecture and produces an LLM-optimized threat model document that other security skills can reference.
...
| Existing threat model   | Path to existing `output/threat-model.md` if updating | No                               |
...
2. **Map components and services**
   ...
   - Look for `apps/`, `services/`, `packages/` or other common directories used in coding patterns
...
- [ ] `output/threat-model.md` exists with all sections populated
- [ ] `output/security-config.json` exists with valid JSON
...
test -s output/threat-model.md && echo "✓ Threat model exists"

# Verify config is valid JSON
cat output/security-config.json | jq . > /dev/null && echo "✓ Config is valid JSON"

# Check threat model has key sections
grep -q "## 1. System Overview" output/threat-model.md && echo "✓ Has System Overview"
grep -q "## 5. Threat Analysis" output/threat-model.md && echo "✓ Has Threat Analysis"
grep -q "## 6. Vulnerability Pattern Library" output/threat-model.md && echo "✓ Has Pattern Library"
```
